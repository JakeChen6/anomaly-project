...
def calc_portfolios(lb):
    ...
    for month in signals.DATE.unique():
        ...
        composite = rows.COM.sort_values()
        num_in_decile = composite.size // 10  # number of stocks in a decile
        winner_threshold = composite.iloc[-num_in_decile]  # threshold of bottom decile 
        loser_threshold = composite.iloc[num_in_decile-1]  # threshold of top decile
        winners[month] = composite[composite >= winner_threshold].index.tolist()
        losers[month] = composite[composite <= loser_threshold].index.tolist()
