...
def calc_portfolios():
    ...
    for fyear in signals['fyear'].unique():
        ...
        xfin = df['xfin'].sort_values()

        # drop outliers?

        num_in_decile = xfin.size // 10  # number of stocks in a decile
        winner_threshold = xfin.iloc[-num_in_decile]  # threshold of bottom decile
        loser_threshold = xfin.iloc[num_in_decile-1]  # threshold of top decile
        winners[month] = xfin[xfin >= winner_threshold].index.tolist()
        losers[month] = xfin[xfin <= loser_threshold].index.tolist()
