...
def calc_portfolios(lb):
    ...
    for month in signals.DATE.unique():
        ...
        adj_shrout = rows.SHR.sort_values()
        num_in_decile = adj_shrout.size // 10  # number of stocks in a decile
        winner_threshold = adj_shrout.iloc[-num_in_decile]  # threshold of bottom decile 
        loser_threshold = adj_shrout.iloc[num_in_decile-1]  # threshold of top decile
        winners[month] = adj_shrout[adj_shrout >= winner_threshold].index.tolist()
        losers[month] = adj_shrout[adj_shrout <= loser_threshold].index.tolist()
