...
def calc_portfolios():
    ...
    signals.rename(columns={'signal': 'ratio'}, inplace=True)
    ...
    for fyear in signals['fyear'].unique():
        ...
        ratio = df['ratio'].sort_values()

        # drop outliers?

        num_in_decile = ratio.size // 10  # number of stocks in a decile
        winner_threshold = ratio.iloc[-num_in_decile]  # threshold of bottom decile
        loser_threshold = ratio.iloc[num_in_decile-1]  # threshold of top decile
        winners[month] = ratio[ratio >= winner_threshold].index.tolist()
        losers[month] = ratio[ratio <= loser_threshold].index.tolist()
