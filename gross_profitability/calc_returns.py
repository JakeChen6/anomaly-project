...
def calc_portfolios():
    ...
    for fyear in signals['fyear'].unique():
        ...
        gp = df['gp'].sort_values()

        # drop outliers?

        num_in_decile = gp.size // 10  # number of stocks in a decile
        winner_threshold = gp.iloc[-num_in_decile]  # threshold of bottom decile
        loser_threshold = gp.iloc[num_in_decile-1]  # threshold of top decile
        winners[month] = gp[gp >= winner_threshold].index.tolist()
        losers[month] = gp[gp <= loser_threshold].index.tolist()
