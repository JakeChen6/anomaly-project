...
def calc_portfolios():
    ...
    for fyear in signals['fyear'].unique():
        ...
        diff = df['diff'].sort_values()

        # drop outliers?

        num_in_decile = diff.size // 10  # number of stocks in a decile
        winner_threshold = diff.iloc[-num_in_decile]  # threshold of bottom decile
        loser_threshold = diff.iloc[num_in_decile-1]  # threshold of top decile
        winners[month] = diff[diff >= winner_threshold].index.tolist()
        losers[month] = diff[diff <= loser_threshold].index.tolist()
