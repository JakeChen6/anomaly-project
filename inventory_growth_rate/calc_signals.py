...
funda_permno = funda_permno.reindex(
    columns=['gvkey', 'lpermno', 'datadate', 'fyear', 'at', 'invt'])
...
for fyear in fyears:  # calculate the asset growth of all the stocks in each fiscal year
    ...
    df_join = df.join(
        df_last_fyear[['gvkey', 'invt']].set_index('gvkey'), on='gvkey', rsuffix='_last_fyear')
    df_join['growth'] = df_join['invt'] / df_join['invt_last_fyear'] - 1  # ratio minus one
