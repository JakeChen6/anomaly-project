...
def calc_portfolios(lb):
    ...
    for month in signals.DATE.unique():
        ...
        lagged_rets = rows.RET.sort_values()
        num_in_decile = lagged_rets.size // 10  # number of stocks in a decile
        winner_threshold = lagged_rets.iloc[-num_in_decile]  # threshold of bottom decile 
        loser_threshold = lagged_rets.iloc[num_in_decile-1]  # threshold of top decile
        winners[month] = lagged_rets[lagged_rets >= winner_threshold].index.tolist()
        losers[month] = lagged_rets[lagged_rets <= loser_threshold].index.tolist()
...
def calc_monthly_rets(*args):
    ...
    for current_month in DATE_RANGE:
        ...
        for previous_month in sorted_months[sorted_months < current_month][-hd:]:
            ...
            current_month_rets['ls'] += lret - wret  # long loser, short winner
