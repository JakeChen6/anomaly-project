...
def calc_portfolios():
    ...
    for i, date in enumerate(DATE_RANGE):
        ...
        cta = df['signal-cta'].sort_values()

        # drop outliers?

        num_in_decile = cta.size // 10  # number of stocks in a decile
        winner_threshold = cta.iloc[-num_in_decile]  # threshold of bottom decile
        loser_threshold = cta.iloc[num_in_decile-1]  # threshold of top decile
        winners[last_month] = cta[cta >= winner_threshold].index.tolist()
        losers[last_month] = cta[cta <= loser_threshold].index.tolist()
