...
def calc_portfolios(lb):
    ...
    for month in signals.DATE.unique():
        ...
        winners[month] = rows[rows.INDICATOR == 1].index.tolist()
        losers[month] = rows[rows.INDICATOR == -1].index.tolist()
...
def calc_monthly_rets(*args):
    ...
    for i, current_month in enumerate(DATE_RANGE):
        ...
        # for a stock split, buy-and-hold for 12 months until a reverse split is seen.
        w = set()
        l = set()
        for j in range(hd, 0, -1):
            m = DATE_RANGE[i-j]
            if m not in winners.keys():
                continue
            split = winners[m]
            reverse_split = losers[m]
            # handle stock split
            w |= set(split)  # include stocks that had a split factor > 1.25 in the past 12 months
            w -= set(reverse_split)  # exclude stocks that had a reverse split in the past 12 months
            # handle reverse split
            l |= set(reverse_split)  # include stocks that had a reverse split in the past 12 months
            l -= set(split)  # exclude stocks that had a split factor > 1.25 in the past 12 months

        data = MSF[(MSF.DATE == current_month) & (MSF.RET.notna())]
        data = data.set_index('PERMNO')

        wret = calc_port_ret(list(w), data)  # winner's return in the current month
        lret = calc_port_ret(list(l), data)  # loser's return in the current month
