...
HISTORY_LIMIT = 50  # at least 50 daily returns
...
def calc_beta(month, lb):
    ...
    # get data in the past 12 month and apply history constraint
    one_year_ago = DATE_RANGE[DATE_RANGE <= month][-12]
    month_begin = MONTHBEGIN.rollback(one_year_ago).to_datetime64()
    ...
    mktrf = MKTRF[(MKTRF.index >= month_begin) &
                  (MKTRF.index <= month)]
    mktrf = mktrf[mktrf < mktrf.mean()]  # only use daily observations with rm < mu
    var = mktrf.var()  # variance of rm
    ...
    with futures.ProcessPoolExecutor(max_workers=CPU_COUNT) as ex:
        ...
        for elm in to_distribute:
            f = ex.submit(helper_calc_beta, data, mktrf, var, elm)  # schedule the future to run
            wait_for.append(f)
...
def helper_calc_beta(data, mktrf, var, permnos):
    res = {}
    for p in permnos:
        rows = data[data.PERMNO == p].set_index('DATE')
        rets = rows.RET.dropna()
        rets -= FF.loc[rets.index, 'rf']  # stock's excess returns
        res[p] = rets.cov(mktrf) / var
    return res
