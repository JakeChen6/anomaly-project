...
def calc_portfolios(lb):
    ...
    for month in signals.DATE.unique():
        ...
        cum_rets = rows.RET
        deciles = pd.qcut(cum_rets.rank(method='first'), CUT, labels=False)  # cut into quintiles based on signal ranking
        winners[month] = deciles[deciles == CUT-1].index.tolist()
