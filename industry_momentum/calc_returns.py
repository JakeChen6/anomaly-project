...
def calc_portfolios(lb):
    ...
    for m in signals['DATE'].unique():
        ...
        groups = cum_rets.groupby(cum_rets).groups
        sorted_keys = sorted(groups.keys())
        winners[m] = reduce(lambda x, y: x + y, [groups[k].tolist() for k in sorted_keys[-IND_COUNT:]])
        losers[m] = reduce(lambda x, y: x + y, [groups[k].tolist() for k in sorted_keys[:IND_COUNT]])
